...
function doMove(game, dir, action) {
    ...
    if (!(player.State & Enums_1.PLAYER_STATES.STANDING)) {
        game.getTeam().addTrophy(Enums_1.TROPHY_IDS.BOOT_SCOOTER);
        action.outcome.push('Trophy Earned: ' + Enums_1.TROPHY_IDS[Enums_1.TROPHY_IDS.BOOT_SCOOTER]);
    ...
    if (cell.isDirOpen(dir)) {
        ...
        game.getTeam().addTrophy(Enums_1.TROPHY_IDS.YOU_FOUGHT_THE_WALL);
...
function doSee(player, cell, dir) {
    ...
    if (dir == cc2018_ts_lib_1.DIRS.NONE) {
        ret = util_1.format('You are %s in a room with %s to the %s.', pPosture, exitString.indexOf('and') < 0 ? 'an exit' : 'exits', exitString);
    }
    else {
        ret = util_1.format('Just to the %s, you see a room with %s to the %s.', getDirName(dir), exitString.indexOf('and') < 0 ? 'an exit' : 'exits', exitString);
    }
    return ret;
}
